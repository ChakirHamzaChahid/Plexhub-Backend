...
## Phases
1. **Requirements** — **la session principale** (skill `product-management:write-spec`) transforme l'objectif en **user stories + critères d'acceptation** + non-goals + métriques + impact contrat d'API. Sortie : `docs/10-prd-<feature>.md`. Modèle : **`.claude/templates/prd-template.md`** (IDs `US1`/`US1-AC1`, `FR-###`, `SC-###`, Given/When/Then, `[NEEDS CLARIFICATION]` ≤ 3). Les ambiguïtés se traitent en phase 1bis, pas au fil de l'eau. *(Mesuré le 2026-09-24 sur les transcripts Claude Code depuis le 2026-09-12 : **0 invocation** de `cpo`, `cto` et `tech-manager` — la planification se fait déjà dans la session principale. On l'assume : phases 1 et 2 dans **un seul contexte**, sans relais PRD → archi → board entre sous-agents, qui perdrait du contexte à chaque passage. `cpo` n'est lancé que si Chakir le demande.)*
1bis. **Clarifier** *(chemin complet uniquement)* — la session principale applique la skill **`spec-quality` §1** : scan des 9 catégories + catégories du dépôt, puis **≤ 5 questions à choix** posées **une par une** (AskUserQuestion, option recommandée en premier avec sa raison), seulement si la réponse change l'archi, les données, le découpage, les tests ou l'UX. Chaque réponse est écrite **aussitôt** dans le PRD (`## Clarifications` → `### Session <date>` → `- Q : … → R : …` + la section concernée). Rien à fort impact → une ligne « Clarifier : aucune ambiguïté critique », sans question.
2. **Architecte + DAG** — **la session principale** (skill `engineering:system-design`) conçoit l'archi (logique métier dans `services/`/`workers/`, routers `api/` = validation+délégation §2 ; schémas Pydantic v2 aux frontières §3 ; impact schéma SQLite/migrations → **propriétaire `db-migration-specialist`** ; pièges §9) + **contrats d'API** (OpenAPI). Puis elle découpe (skill `sprint-planner`) en **sous-issues dépendantes** sur `docs/31-board.md` (colonnes Status/Depends on/Owner/Safe-Risky + **US·FR couverts** + **fichiers/modules** par ticket, tickets groupés par story avec un **checkpoint** par story — format dans `sprint-planner`), arêtes de dépendance explicites. **GATE** : présente le découpage + l'ordre d'exécution (séquentiel) ; attends le « go ». Le GATE se tient **après la phase 2bis** et montre son rapport (0 CRITICAL exigé).
2bis. **Analyser** *(chemin complet uniquement, avant le GATE)* — un sous-agent **neuf, en lecture seule** (`tech-lead`, couple noté par la grille, `sonnet`·`medium` par défaut ; consigne explicite : ne modifier aucun fichier) applique la skill **`spec-quality` §2** au PRD + lignes board (+ ADR) : 6 passes (doublons, ambiguïtés, sous-spécification, loi de la maison et pièges connus, couverture, incohérences), sévérités, **matrice FR → tickets**, ≤ 50 constats. Si la grille donne **C2 = 2 ou C3 = 2**, la **même** invocation challenge aussi l'archi (une seule relecture, pas deux). Si **C3 ≥ 1**, il ajoute la checklist de qualité des exigences (§3). **Un CRITICAL bloque le GATE** ; un HIGH est corrigé ou accepté par Chakir au GATE (tracé dans « Clarifications ») ; c'est la session principale qui corrige PRD et board, l'analyste ne touche à rien.
