...
**Mesure du roster (13 → 25/09, transcripts Claude Code)** : 0 lancement pour les rôles de pilotage et de QA `ceo`, `cpo`, `cto`, `tech-manager`, `qa-engineer` — le merge et la QA ont été faits par la session principale. **Décision de Chakir (2026-09-25) : on conserve et on mesure.** Réévaluation le **2026-10-16** avec `python .claude/tools/cost-tracker.py report --weeks 3` : un rôle toujours à 0 est soit archivé (fiche + jumeau `-high`), soit réécrit comme étape de la session principale. Les agents d'usage rare par nature (audits, cleanroom, spécialistes domaine, `observability-analyst`) ne sont pas concernés par ce critère.
