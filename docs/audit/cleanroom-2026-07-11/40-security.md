...
### CR-S03 — Xtream account passwords stored in plaintext at rest (and in unencrypted backups) (P2)

**Where:** `app/models/database.py:136` (`password = Column(Text, nullable=False)`); written plaintext at `app/api/accounts.py:72`, `app/main.py:163`; snapshotted by the online-backup cron (`app/main.py:309-324` → `app/scripts/backup_db`) to `BACKUP_DIR` with no encryption.
