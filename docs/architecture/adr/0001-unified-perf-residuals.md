...
**Décision.** Ajouter un paramètre **optionnel** `cursor` aux listes brutes (`/movies`, `/shows`, `/episodes`) qui, **quand il est fourni ET que `sort ∈ {added_desc, added_asc}`** (clé mono-colonne avec tie-break stable `(added_at, guid)`), remplace l'OFFSET par un seek (`WHERE (added_at, guid) < :cursor`). `offset` reste pleinement fonctionnel (défaut). Le curseur opaque de la page est renvoyé dans la réponse (`next_cursor`).
