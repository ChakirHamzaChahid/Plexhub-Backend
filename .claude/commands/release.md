---
description: Workflow Release — tests verts → bump APP_VERSION → tag vX.Y.Z sur main → build+push image Docker (GHCR via .github/workflows/docker.yml) → vérif. Délègue à release-manager. Risky → needs-approval.
...
## Phases
1. **Préconditions** — branche `main` propre, `pytest -v` **vert**, serveur boote (`uvicorn app.main:app` + `GET /api/health` 200), `gh` authentifié, `docker.yml` présent. Aucun secret en clair.
2. **Version** — résoudre la version (argument sinon `APP_VERSION` dans `app/main.py`), vérifier qu'elle est **strictement > la dernière publiée** (sinon le tag/l'image n'apportent rien). **Bump `APP_VERSION`** (`app/main.py`) si nécessaire ; commit du bump sur `main`.
3. **Tag** — créer le tag **`vX.Y.Z` sur `main`** (annoté). Ne **jamais** écraser/déplacer un tag existant.
...
- **Traçabilité** : version commitée sur `main` ; rapport final (version, tag, URL image GHCR, résultat smoke `docker run` + `/api/health`).
